...
func Init() (f flags.Flags, rs repos.Repos, st *stat.Stat, ti *timer.Timer) {
	...
	efih := emoji.Get("FlagInHole")                                    //
	brf.Printv(f, "%v parsing flags", efih)                            // print parsing flags
	ef := emoji.Get("Flag")                                            //
	fm := f.Mode                                                       //
	ts := ti.Split()                                                   //
	tt := ti.Time()                                                    //
	brf.Printv(f, "%v running in '%v' mode {%v / %v}", ef, fm, ts, tt) // print mode
	eb := emoji.Get("Books")                                           //
	brf.Printv(f, "%v reading ~/.gisrc.json", eb)                      // print reading config
	...
	ts = ti.Split()                                                    //
	tt = ti.Time()                                                     //
	brf.Printv(f, "%v read %v {%v / %v}", ebs, fc, ts, tt)             // print read config
...
func main() {
	...
	rs.VerifyChanges(f, st, t)    // verify changes, confirm with user
	rs.SubmitChanges(f, st, t)    // submit changes (async)
}
